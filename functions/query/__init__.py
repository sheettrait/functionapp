...
# SQL Server access token attribute key for pyodbc
SQL_COPT_SS_ACCESS_TOKEN = 1256

# Allowed tables and columns (whitelist to mitigate SQL injection)
ALLOWED_TABLES: Dict[str, Dict[str, Any]] = {
    "Patient": {
        "columns": [
            "PatientID",
...
def build_query(table: str, body: Dict[str, Any]) -> Tuple[str, List[Any]]:
    ...
    # OFFSET/FETCH requires ORDER BY; keep stable ordering for deterministic pagination
    query = f"SELECT {', '.join(columns)} FROM {table}{where_clause}{order_clause} OFFSET 0 ROWS FETCH NEXT ? ROWS ONLY"
    params.append(limit)
